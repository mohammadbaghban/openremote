...
import { VideoWidgetConfig } from "../widgets/video-widget";
...
export class VideoSettings extends AssetWidgetSettings {
    ...
    protected render(): TemplateResult {
        return html`
            <div>
                ...
                <!-- Video settings -->
                <settings-panel displayName="dashboard.videoSettings" expanded="${true}">
                    <div>
                        <or-mwc-input style="width: 100%;" type="${InputType.TEXT}" label="${i18next.t('dashboard.videoUrl')}" .value="${this.widgetConfig.videoPath}"
                                      @or-mwc-input-changed="${(ev: OrInputChangedEvent) => this.onVideoUrlUpdate(ev)}"
                        ></or-mwc-input>
                    </div>

                    <div style="margin-top: 12px;">
                        <div style="margin-bottom: 6px; font-size: 0.9em; color: var(--or-app-color2);">
                            Video URL attribute (optional)
                        </div>
                        <attributes-panel .attributeRefs="${this.widgetConfig.videoUrlAttributeRef ? [this.widgetConfig.videoUrlAttributeRef] : []}"
                                          onlyDataAttrs="${true}" multi="${false}"
                                          @attribute-select="${(ev: AttributesSelectEvent) => this.onVideoUrlAttributeSelect(ev)}"
                        ></attributes-panel>
                        <div style="margin-top: 6px; color: gray; font-size: 0.85em;">
                            If set, this attribute's string value will be used as the video URL and overrides the static Video URL above.
                        </div>
                    </div>
                </settings-panel>
            </div>
    ...
    protected onVideoUrlUpdate(ev: OrInputChangedEvent) {
        this.widgetConfig.videoPath = ev.detail.value;
        this.notifyConfigUpdate();
    }

    protected onVideoUrlAttributeSelect(ev: AttributesSelectEvent) {
        const refs: AttributeRef[] = ev.detail.attributeRefs || [];
        const newRef: AttributeRef | undefined = refs.length > 0 ? refs[0] : undefined;
        const oldRef: AttributeRef | undefined = this.widgetConfig.videoUrlAttributeRef;
        const unchanged = (
            (!oldRef && !newRef) ||
        ...
        this.widgetConfig.videoUrlAttributeRef = newRef;
        this.notifyConfigUpdate();
    }

    // updates coordinate map according to the attributeRef entries per id
    updateCoordinateMap(config: VideoWidgetConfig) {
    ...
    private draftCoordinateEntries(config: VideoWidgetConfig): TemplateResult[] {
        const min = 0;
        const max = 100;
