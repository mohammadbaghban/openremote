...
export interface VideoAssetMarker {
...
export interface VideoWidgetConfig extends WidgetConfig {
    ...
    markers: VideoAssetMarker[];
    showTimestampControls: boolean;
    videoPath: string;
    // Optional: bind video URL to a string attribute; if set, this overrides videoPath
    videoUrlAttributeRef?: AttributeRef;
}

function getDefaultWidgetConfig(): VideoWidgetConfig {
    return {
        attributeRefs: [],
...
export class VideoWidget extends OrAssetWidget {

    protected readonly widgetConfig!: VideoWidgetConfig;

    protected _attributeSubscriptionId?: string;

    protected _videoVersion: number = 0;

    protected getAllAttributeRefs(): AttributeRef[] {
        const attributeRefs = this.widgetConfig?.attributeRefs || [];
        return this.widgetConfig?.videoUrlAttributeRef ? [...attributeRefs, this.widgetConfig.videoUrlAttributeRef] : attributeRefs;
    }
    ...
    protected async _subscribeAttributeEvents() {
        try {
            ...
            if (!events) {
                console.log("VideoWidget: No event provider available so cannot subscribe");
                return;
            }
            this._attributeSubscriptionId = await events.subscribeAttributeEvents(refs, true, (event) => {
                ...
                const vidRef = this.widgetConfig?.videoUrlAttributeRef;
                if (vidRef && ref.id === vidRef.id && ref.name === vidRef.name) {
                    this._videoVersion++;
                }
                this.requestUpdate();
            });
        } catch (e) {
            console.warn("VideoWidget: failed to subscribe attribute events", e);
        }
    }
    ...
    static getManifest(): WidgetManifest {
        return {
            ...
            minColumnWidth: 1,
            minColumnHeight: 1,
            getContentHtml(config: VideoWidgetConfig): OrWidget {
                return new VideoWidget(config);
            },
            getSettingsHtml(config: VideoWidgetConfig): WidgetSettings {
                return new VideoSettings(config);
            },
            getDefaultConfig(): VideoWidgetConfig {
    ...
    willUpdate(changedProps: PropertyValues) {
        if(changedProps.has('widgetConfig') && this.widgetConfig) {
            const prevConfig = changedProps.get('widgetConfig') as VideoWidgetConfig | undefined;

            const attributeRefs = this.widgetConfig.attributeRefs || [];
            const allRefs: AttributeRef[] = this.widgetConfig.videoUrlAttributeRef ? [...attributeRefs, this.widgetConfig.videoUrlAttributeRef] : attributeRefs;
            const missingAssets = allRefs?.filter((attrRef: AttributeRef) => !this.isAttributeRefLoaded(attrRef));
            if (missingAssets.length > 0) {
                this.loadAssets();
            }
            if (prevConfig && prevConfig.videoPath !== this.widgetConfig.videoPath) {
                this._videoVersion++;
            }
            this._unsubscribeAttributeEvents();
            this._subscribeAttributeEvents();
    ...
    protected handleMarkerPlacement(config: VideoWidgetConfig) {
    ...
    protected getVideoPathFromAttribute(): string | undefined {
        const vidRef = this.widgetConfig.videoUrlAttributeRef;
        if (!vidRef || !this.loadedAssets || this.loadedAssets.length === 0) {
            return undefined;
        }
        const asset = this.loadedAssets.find(a => a.id === vidRef.id);
        if (!asset || !asset.attributes) {
            return undefined;
        }
        const attribute = asset.attributes[vidRef.name!];
        if (!attribute) {
            return undefined;
        }
        const descriptors = AssetModelUtil.getAttributeAndValueDescriptors(asset.type, vidRef.name, attribute);
    ...
    protected render(): TemplateResult {
        const attrPath = this.getVideoPathFromAttribute();
        const rawPath = attrPath ?? this.widgetConfig.videoPath;
        const videoPath = this._withCacheBust(rawPath, this._videoVersion);
        return html`
            <div id="video-wrapper">
                ${when(videoPath, () => html`
                    <div id="video-container">
                        ${repeat([videoPath], (key) => key, () => html`
                            <video id="video-content" src="${videoPath}" controls></video>
                        `)}
                        <div>
                ...
                `, () => html`
                    <span><or-translate value="dashboard.noVideoSelected"></or-translate></span>
                `)}
            </div>
        `;
    }
